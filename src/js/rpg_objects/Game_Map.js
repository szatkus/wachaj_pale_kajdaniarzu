...
Game_Map.prototype.roundX = function(x) {
    return this.isLoopHorizontal() ? x.mod(this.width()) : x.clamp(0, this.width() - 1);
};

Game_Map.prototype.roundY = function(y) {
    return this.isLoopVertical() ? y.mod(this.height()) : y.clamp(0, this.height() - 1);
};
...
Game_Map.prototype.refreshTileset = function() {
    ...
    if (this._tileset) {
        this._tileset.bitmaps = this.tileset().tilesetNames.map(function(tilesetName) {
            return ImageManager.loadTileset(tilesetName);
        });
    }
};
...
Game_Map.prototype.changeParallax = function(name, loopX, loopY, sx, sy) {
    ...
    this._parallaxZero = ImageManager.isZeroParallax(this._parallaxName);
    if (loopX) {
        this._parallaxLoopX = true;
        this._parallaxSx = 0;
    }
    if (loopY) {
        this._parallaxLoopY = true;
        this._parallaxSy = 0;
    }
    if (sx) {
        this._parallaxSx = sx;
...
Game_Map.prototype.changeFog = function(name, hue, opacity, blendMode, speed, zoom) {
    //
};
...
Game_Map.prototype.checkLayeredTilesFlags = function(x, y, flags) {
    var tiles = this.layeredTiles(x, y);
    for (var i = 0; i < tiles.length; i++) {
        var tileId = tiles[i];
        if ((this.tilesetFlags()[tileId] & flags) !== 0) {
            return true;
        }
    }
    return false;
...
Game_Map.prototype.terrainTag = function(x, y) {
    if (this.isValid(x, y)) {
        var tiles = this.layeredTiles(x, y);
        for (var i = 0; i < tiles.length; i++) {
            var tag = this.tilesetFlags()[tiles[i]] >> 12;
            if (tag > 0) {
                return tag;
            }
        }
...
Game_Map.prototype.startEncounterEffect = function() {
    this._interpreter.startEncounterEffect();
};

Game_Map.prototype.terminateEncounterEffect = function() {
    this._interpreter.terminateEncounterEffect();
};
...
Game_Map.prototype.tileEventsXy = function(x, y) {
    return this.events().filter(function(event) {
        return event.isTile() && event.pos(x, y);
    });
};
...
Game_Map.prototype.isAirshipLandOk = function(x, y) {
    return this.checkPassage(x, y, 0x0100) && this.checkPassage(x, y, 0x0f);
};

Game_Map.prototype.boatPassable = function(x, y) {
    return this.checkPassage(x, y, 0x0200);
};

Game_Map.prototype.shipPassable = function(x, y) {
    return this.checkPassage(x, y, 0x0400);
};

Game_Map.prototype.airshipLandOk = function(x, y) {
    return this.checkPassage(x, y, 0x0100) && this.checkPassage(x, y, 0x0f);
};
