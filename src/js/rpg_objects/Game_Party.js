...
Game_Party.prototype.setupBattleTestEquips = function() {
    $dataWeapons.forEach(function(weapon) {
        if (weapon && weapon.name.length > 0) {
            this.gainItem(weapon, this.maxItems(weapon));
        }
    }, this);
    $dataArmors.forEach(function(armor) {
        if (armor && armor.name.length > 0) {
            this.gainItem(armor, this.maxItems(armor));
        }
    }, this);
};
...
Game_Party.prototype.hasItem = function(item, includeEquip) {
    ...
    if (includeEquip && this.isAnyMemberEquipped(item)) {
        return true;
    }
    return false;
};
