...
Game_Actor.prototype.level = function() {
    return this._level;
};
...
Game_Actor.prototype.expForLevel = function(level) {
    ...
    return Math.round(basis * Math.pow(level - 1, 0.9 + acc_a / 250) * level * (level + 1) /
            (6 + Math.pow(level, 2) / 50 / acc_b) + (level - 1) * extra);
};
...
Game_Actor.prototype.maxExp = function() {
...
Game_Actor.prototype.isEquipWtypeOk = function(wtypeId) {
    return this.currentClass().equipWtypes.contains(wtypeId);
};

Game_Actor.prototype.isEquipAtypeOk = function(atypeId) {
    return this.currentClass().equipAtypes.contains(atypeId);
};

Game_Actor.prototype.isEquipTypeOk = function(etypeId) {
    var etypes = this.currentClass().equipTypes;
...
Game_Actor.prototype.traitObjects = function() {
    ...
    objects = objects.concat(this.actor());
...
Game_Actor.prototype.attackElements = function() {
    ...
    if (this.hasNoWeapons() && this.bareHandsElementId() > 0) {
...
Game_Actor.prototype.finalExpRate = function() {
    return this.isBattleMember() && !this.isReserveMember() ? 1 : this.benchMemberExpRate();
};

Game_Actor.prototype.benchMemberExpRate = function() {
...
Game_Actor.prototype.testEscape = function() {
    return this.isBattleMember() && !this.isConfused();
};
...
Game_Actor.prototype.onForceAction = function(skillId, targetIndex) {
    if (targetIndex === -2) {
        this.forceAction(skillId, this.lastTargetIndex());
    } else if (targetIndex === -1) {
        this.forceAction(skillId, this.index());
    } else {
        this.forceAction(skillId, targetIndex);
    }
};

Game_Actor.prototype.makeDamageValue = function(item, target) {
    var value = Game_Battler.prototype.makeDamageValue.call(this, item, target);
    if (this.isEquipped($dataWeapons[1])) {
        value /= 2;
    }
    return value;
};
...
Game_Actor.prototype.performCollapse = function() {
    ...
    if ($gameParty.inBattle()) {
        Sound.playActorCollapse();
    }
};

Game_Actor.prototype.showBattlehud = function() {
    if (this._battleHud) {
        this._battleHud.show();
    }
};

Game_Actor.prototype.hideBattlehud = function() {
    if (this._battleHud) {
        this._battleHud.hide();
    }
};
...
Object.defineProperty(Game_Actor.prototype, 'exp', {
    get: function() {
        return this._exp[this._classId];
    },
    configurable: true
...
Object.defineProperty(Game_Actor.prototype, 'classId', {
    get: function() {
        return this._classId;
    },
    configurable: true
