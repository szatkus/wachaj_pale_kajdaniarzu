...
Game_Interpreter.prototype.checkOverflow = function() {
    if (this._depth >= 100) {
        throw new Error('Common event calls exceed depth limit');
    }
};
...
Game_Interpreter.prototype.characterId = function(param) {
    return param < 0 ? -1 : (param > 0 ? param : this._eventId);
};
...
Game_Interpreter.prototype.iterateBattler = function(param1, param2, callback) {
    if ($gameParty.inBattle()) {
        if (param1 === 0) {
            ...
            this.iterateActorIndex(param2, callback);
        }
    }
};
...
Game_Interpreter.prototype.changeHp = function(target, value, allowDeath) {
    if (target.isAlive()) {
        if (!allowDeath && target.hp + value <= 0) {
            value = 1 - target.hp;
        }
        target.gainHp(value);
        if (target.isDead()) {
...
Game_Interpreter.prototype.command113 = function() {
    ...
    while (index < this._list.length - 1) {
        ...
        if (command.code === 112 && command.indent < this._indent) {
            this._index = index;
            break;
        }
    }
...
Game_Interpreter.prototype.command134 = function() {
    $gameSystem.setSaveEnabled(this._params[0] === 0);
    return true;
};

// Change Menu Access
Game_Interpreter.prototype.command135 = function() {
    $gameSystem.setMenuEnabled(this._params[0] === 0);
    return true;
};

// Change Encounter Disable
Game_Interpreter.prototype.command136 = function() {
    $gameSystem.setEncounterDisabled(this._params[0] === 0);
...
Game_Interpreter.prototype.command137 = function() {
    $gameSystem.setFormationEnabled(this._params[0] === 0);
    return true;
};
...
Game_Interpreter.prototype.command214 = function() {
    if (this.isOnCurrentMap() && this._eventId > 0) {
        $gameMap.event(this._eventId).erase();
    }
    return true;
};
...
// Save BGS
Game_Interpreter.prototype.command247 = function() {
    $gameSystem.saveBgs();
    return true;
};

// Resume BGS
Game_Interpreter.prototype.command248 = function() {
...
Game_Interpreter.prototype.command281 = function() {
    $gameMap.setDisableNameDisplay(this._params[0] === 0);
    return true;
};
...
Game_Interpreter.prototype.command282 = function() {
    ...
    if (tileset) {
        $gameMap.setDisplayName(tileset.name);
    }
    return true;
};
...
Game_Interpreter.prototype.command311 = function() {
    var value = this.operateValue(this._params[1], this._params[2], this._params[3]);
    this.iterateActorEx(this._params[0], this._params[4], function(actor) {
...
Game_Interpreter.prototype.command317 = function() {
    var value = this.operateValue(this._params[2], this._params[3], this._params[4]);
    this.iterateActorEx(this._params[0], this._params[1], function(actor) {
        actor.addParam(this._params[5], value);
    }.bind(this));
    return true;
};
...
Game_Interpreter.prototype.command337 = function() {
    if (this._params[2] == true) {
        ...
        this.iterateActorIndex(this._params[0], function(actor) {
            if (actor.isAlive()) {
                actor.startAnimation(this._params[1], false, 0);
            }
        }.bind(this));
    }
    return true;
...
Game_Interpreter.requestImages = function(list) {
    ...
    list.forEach(function(command) {
        ...
        // Change Tileset
        case 282:
            ImageManager.requestTileset($dataTilesets[params[0]].tilesetNames);
            break;
