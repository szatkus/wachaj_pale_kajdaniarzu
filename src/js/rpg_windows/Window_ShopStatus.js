...
Window_ShopStatus.prototype.drawEquipInfo = function(x, y) {
    ...
    for (var i = 0; i < members.length; i++) {
        this.drawActorEquipInfo(x, y + this.lineHeight() * i, members[i]);
    }
};
...
Window_ShopStatus.prototype.drawActorEquipInfo = function(x, y, actor) {
    ...
    this.drawItemName(item1, x + 128, y);
...
Window_ShopStatus.prototype.currentEquippedItem = function(actor, etypeId) {
    ...
    for (var j = 0; j < list.length; j++) {
        var item = list[j];
        if (item && item.params[paramId] > bestPerformance) {
            bestPerformance = item.params[paramId];
            bestItem = item;
        }
    }
    return bestItem;
...
Window_ShopStatus.prototype.updatePage = function() {
    if (this.isPageChangeEnabled() && this.isPageChangeCalled()) {
        this.changePage();
    }
};
...
Window_ShopStatus.prototype.isPageChangeCalled = function() {
    return Input.isTriggered('shift') || Input.isTriggered('pagedown') ||
