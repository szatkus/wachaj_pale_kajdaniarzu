...
Window_Base.prototype.standardFontFace = function() {
    if ($gameSystem.isJapanese()) {
        return 'ヒラギノ角ゴ ProN W3', 'Hiragino Kaku Gothic ProN', 'メイリオ', 'Meiryo', 'ＭＳ Ｐゴシック', 'MS PGothic';
    } else {
        return 'GameFont';
    }
};
...
Window_Base.prototype.mpCostColor = function(actor) {
    return this.textColor(23);
};
...
Window_Base.prototype.tpCostColor = function(actor) {
    return this.textColor(29);
};
...
Window_Base.prototype.contentsOpacity = function() {
    return this.contents.paintOpacity;
};
...
Window_Base.prototype.processDrawIcon = function(iconIndex, textState) {
    ...
    this.contents.blt(iconBitmap, sx, sy, pw, ph, textState.x + 2, textState.y + 2);
...
Window_Base.prototype.calcTextHeight = function(textState, all) {
    ...
    for (var i = 0; i < maxLines; i++) {
        ...
        textHeight += Math.max(textHeight, this.contents.fontSize);
    }
...
Window_Base.prototype.canvasToLocalX = function(x, y) {
    var node = this;
    while (node) {
        x -= node.x;
        y -= node.y;
...
Window_Base.prototype.canvasToLocalY = function(x, y) {
    var node = this;
    while (node) {
        x -= node.x;
        y -= node.y;
