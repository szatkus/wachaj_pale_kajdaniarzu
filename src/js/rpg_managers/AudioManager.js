...
AudioManager.playMe = function(me) {
    ...
    if (me.name) {
        ...
        this._meBuffer.addEndListener(this.stopMe.bind(this));
    }
};
...
AudioManager.stopMe = function() {
    if (this._meBuffer) {
        this._meBuffer.stop();
        this._meBuffer = null;
        if (this._currentBgm && this._bgmBuffer) {
...
AudioManager.loadStaticSe = function(se) {
    if (se.name && !this.isStaticSe(se)) {
        ...
        this.cleanupStaticSe();
    }
};

AudioManager.cleanupStaticSe = function() {
    for (var i = 0; i < this._staticBuffers.length; i++) {
        var buffer = this._staticBuffers[i];
        if (!buffer.isPlaying()) {
            if (buffer._reservedSeName) {
                this._staticBuffers.splice(i, 1);
                i--;
            }
        }
    }
...
AudioManager.makeEmptyAudioObject = function() {
    return { name: '', volume: 0, pitch: 100 };
};
...
AudioManager.shouldUseHtml5Audio = function(){
    // The only case where we wanted HTML5 Audio for BGM was for iOS.
    // However, iOS now supports Web Audio API, so we no longer need this.
    return false;
};
...
AudioManager.checkErrors = function() {
    ...
    for (var j = 0; j < this._staticBuffers.length; j++) {
        this.checkWebAudioError(this._staticBuffers[j]);
    }
};
