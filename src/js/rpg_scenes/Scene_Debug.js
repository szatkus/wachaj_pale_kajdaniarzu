...
Scene_Debug.prototype.refreshDebugHelpWindow = function() {
    this._debugHelpWindow.contents.clear();
    if (this._editWindow.active) {
        var id = this._rangeWindow.topId() + this._editWindow.index();
        if (this._rangeWindow.mode() === 'switches') {
            this._debugHelpWindow.drawText(this.helpText(), 0, 0, this._debugHelpWindow.contentsWidth());
        }
    }
};
